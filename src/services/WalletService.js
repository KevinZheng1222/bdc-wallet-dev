import {Aes, key, PrivateKey, TransactionBuilder, TransactionHelper} from 'bdcjs';
import {Apis} from 'bdcjs-ws';
 ...
 */
const get_wallets = () => {
    let wallets = localStorage.getItem(`bdc_wallets_${Apis.instance().chain_id}`);
    if (!wallets) {
        wallets = [];
    } else {
...
const bak_wallet = () => {
    ...
    if (!(localStorage.getItem(`bdc_wallets_bak3_${Apis.instance().chain_id}`))) {
        localStorage.setItem(`bdc_wallets_bak3_${Apis.instance().chain_id}`, JSON.stringify(localStorageWallets));
    }
};
...
const merge_wallets = () => {
    return new Promise((resolve, reject) => {
        ...
        resolve(IndexedDB.openDB(`bdc_wallets_${Apis.instance().chain_id}`, 1, {
            name: 'wallet',
            key: 'walletKey'
        }).then((db) => {
            let walletDB = db;
            return IndexedDB.getData(walletDB, 'wallet', `bdc_wallets_${Apis.instance().chain_id}`).then((res) => {
                if (res) {
                    let localStorageWallets = get_wallets();
                    let unionWallets = unionBy(localStorageWallets, res.value, 'account');
                    localStorage.setItem(`bdc_wallets_${Apis.instance().chain_id}`, JSON.stringify(unionWallets));
                }
...
const set_wallets_db = (wallets) => {
    return new Promise((resolve, reject) => {
        let walletDB = null;
        return IndexedDB.openDB(`bdc_wallets_${Apis.instance().chain_id}`, 1, walletDB, {
            name: 'wallet',
            key: 'walletKey'
        }).then((db) => {
            let walletDB = db;
            return IndexedDB.putJSON(walletDB, 'wallet', {
                walletKey: `bdc_wallets_${Apis.instance().chain_id}`,
                value: wallets
            }).then(() => {
                IndexedDB.closeDB(walletDB);
                resolve();
...
const set_wallet_native = (wallets) => {
    return new Promise((resolve, reject) => {
        ...
        cordova.exec(function () { //eslint-disable-line
            ...
            reject();
        }, pluginName, 'set', [`bdc_wallets_${Apis.instance().chain_id}`, JSON.stringify(wallets)]);
    });
};
...
const get_wallet_native = () => {
    return new Promise((resolve, reject) => {
        ...
        cordova.exec(function (result) { //eslint-disable-line
            ...
            reject();
        }, pluginName, 'get', [`bdc_wallets_${Apis.instance().chain_id}`]);
    });
};
...
const set_wallets = (wallets) => {
    return new Promise((resolve, reject) => {
        localStorage.setItem(`bdc_wallets_${Apis.instance().chain_id}`, JSON.stringify(wallets));
        try {
...
const get_wallet_index = () => {
    ...
    let index = localStorage.getItem(`bdc_wallet_index_${Apis.instance().chain_id}`);
    if (!index) {
        return 0;
    }
    index = Number(index);
...
const set_wallet_index = (index) => {
    index = Number(index);
    let wallets = get_wallets();
    localStorage.setItem(`bdc_wallet_index_${Apis.instance().chain_id}`, Math.min(wallets.length - 1, index));
};
...
const get_disclaimer_accepted = () => {
    let result = localStorage.getItem(`bdc_disclaimer_accepted_${Apis.instance().chain_id}`);
...
const set_disclaimer_accepted = (accepted) => {
    if (accepted) {
        localStorage.setItem(`bdc_disclaimer_accepted_${Apis.instance().chain_id}`, accepted ? 1 : 0);
    } else {
        localStorage.removeItem(`bdc_disclaimer_accepted_${Apis.instance().chain_id}`);
    }
};
            ...
            return Apis.instance().db_api().exec('get_objects', [account_ids]).then((accounts) => {
                if (accounts.length > 0) {
                    ...
                    if (imported.length > 0) {
                        set_wallets(wallets);
                        localStorage.setItem(`bdc_wallet_index_${Apis.instance().chain_id}`, wallets.length - 1);
                    }
                    return {
                        imported,
                        exist
...
/**
 * fetch account balance of bdc by account name or id
 * @param account_name
 * @returns {bluebird}
 */
